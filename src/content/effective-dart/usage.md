...
description: 유지 관리가 가능한 코드를 작성하기 위해 언어 기능을 사용하기 위한 지침입니다.
nextpage:
...
이 가이드라인은 매일 Dart 코드 본문에서 사용할 수 있습니다. 
라이브러리의 *사용자*는 당신이 여기의 아이디어를 내면화했다는 것을 알아차리지 못할 수 있지만, 
라이브러리의 *유지 관리자*는 확실히 알아차릴 것입니다.
...
### DON'T import libraries that are inside the `src` directory of another package {:#dont-import-libraries-that-are-inside-the-src-directory-of-another-package}
...
### DON'T allow an import path to reach into or out of `lib` {:#dont-allow-an-import-path-to-reach-into-or-out-of-lib}
...
### DON'T explicitly initialize variables to `null` {:#dont-explicitly-initialize-variables-to-null}
...
### DON'T use an explicit default value of `null` {:#dont-use-an-explicit-default-value-of-null}
...
### DON'T use `true` or `false` in equality operations {:#dont-use-true-or-false-in-equality-operations}
...
### DON'T use `.length` to see if a collection is empty {:#dont-use-length-to-see-if-a-collection-is-empty}
...
### DON'T use `cast()` when a nearby operation will do {:#dont-use-cast-when-a-nearby-operation-will-do}
...
### DON'T create a lambda when a tear-off will do {:#dont-create-a-lambda-when-a-tear-off-will-do}
...
### DON'T wrap a field in a getter and setter unnecessarily {:#dont-wrap-a-field-in-a-getter-and-setter-unnecessarily}
...
### DON'T use `late` when a constructor initializer list will do {:#dont-use-late-when-a-constructor-initializer-list-will-do}
...
### DON'T use `const` redundantly {:#dont-use-const-redundantly}
...
### DON'T discard errors from catches without `on` clauses {:#dont-discard-errors-from-catches-without-on-clauses}
...
### DON'T explicitly catch `Error` or types that implement it {:#dont-explicitly-catch-error-or-types-that-implement-it}
...
### PREFER async/await over using raw futures {:#prefer-asyncawait-over-using-raw-futures}
...
### DON'T use `async` when it has no useful effect {:#dont-use-async-when-it-has-no-useful-effect}
...
### DO test for `Future<T>` when disambiguating a `FutureOr<T>` whose type argument could be `Object` {:#do-test-for-futuret-when-disambiguating-a-futureort-whose-type-argument-could-be-object}
